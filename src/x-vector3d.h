...
public:
    ...
    void setAll( FLOAT val ) { x = y = z = val; }
...
public:
    ...
    { FLOAT result = x*rhs.x + y*rhs.y + z*rhs.z; return result; }
    // cross product
    ...
    { FLOAT mag = magnitude(); if( mag == 0 ) return; *this *= 1/mag; }
    // 2d angles
    inline FLOAT angleXY() const
    { return ::atan2( y, x ); }
    inline FLOAT angleYZ() const
    { return ::atan2( z, y ); }
    inline FLOAT angleXZ() const
    { return ::atan2( z, x ); }
...
public: // using the 3-tuple for interpolation
    ...
    inline void interp( FLOAT delta )
    { value = (goal-value)*slew*delta + value; }
    inline void interp2( FLOAT delta ) // time-invariant
    { value = (goal - value) * (1.0f - ::powf(1.0f - slew, delta)) + value; }
    inline void update( FLOAT _goal )
    { goal = _goal; }
    inline void update( FLOAT _goal, FLOAT _slew )
    { goal = _goal; slew = _slew; }
    inline void updateSet( FLOAT _goalAndValue )
    { goal = value = _goalAndValue; }
    inline void updateSet( FLOAT _goalAndValue, FLOAT _slew )
    { goal = value = _goalAndValue; slew = _slew; }
    
public:
    // either use as .x, .y, .z OR .value, .goal, .slew
    union { FLOAT x; FLOAT value; };
    union { FLOAT y; FLOAT goal; };
    union { FLOAT z; FLOAT slew; };
    
public:
    static FLOAT nowhere;
    static FLOAT zero;
};
...
public:
    ...
    iSlew3D( FLOAT x, FLOAT y, FLOAT z, float slew = 0.5 ) : m_slewX(x, x, slew), m_slewY(y, y, slew), m_slewZ(z, z, slew), m_actual(x, y, z) { }
